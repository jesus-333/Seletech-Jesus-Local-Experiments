...
p_value_array = np.zeros((len(t_list), len(group_list), len(group_list)))
...
for i in range(len(t_list)):
    ...
    for j in range(len(group_list)):
        group_1 = spectra_data[spectra_data['test_control'] == group_list[j]]
        group_1 = group_1.loc[:, str(wavelength_min):str(wavelength_max)].to_numpy()

        if average_spectra : group_1 = group_1.mean(0)
         
        for k in range(len(group_list)):
            group_2 = spectra_data[spectra_data['test_control'] == group_list[k]]
            group_2 = group_2.loc[:, str(wavelength_min):str(wavelength_max)]
            if average_spectra : group_2 = group_2.mean(0)
            ...
            print("{} - {} : p-value = {}".format(group_list[j], group_list[k], p_value))
