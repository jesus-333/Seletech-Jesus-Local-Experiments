...
for i in range(len(t_list)):
    ...
    for lamp_power_idx in range(len(lamp_power_list)):
        ...
        if len(tmp_spectra) > 0:
            plant_group_list = list(set(tmp_spectra['test_control']))

            # Plot the spectra for each group
            for idx_plant_group in range(len(plant_group_list)):
                plant_group = plant_group_list[idx_plant_group]
                ...
                ax = axs[idx_plant_group]

                # Plot the average spectra per group
                ax.hist(spectra_to_plot, plot_config['n_bins'], density = normalize_hist,
                        histtype = 'step', label = "Lamp power {}".format(lamp_power),
                        color = lamp_to_color[lamp_power], linewidth = plot_config['linewidth']
                        )
                ...
                if normalize_hist : ax.set_ylim([0, 350000])
    ...
    if plot_config['save_fig'] :
        ...
        path_save += '3_hist_V2_t_{}_w_{}_p_{}_der_{}_mems_{}'.format(t, w, p, deriv, mems_to_plot)
