...
For each group (control, test_150, test_300) visualize the average value of the wavelenght across time.
...
lamp_power = 80 # If None average among all lamp powers. Otherwise select a specific lamp power. Possible values are 50, 60, 70, 80
...
for group in plant_group_list:
    ...
    for i in range(len(t_list)):
        ...
        if lamp_power is not None : data_beans = data_beans[data_beans['lamp_0'] == lamp_power]
    ...
    if plot_config['save_fig']:
        ...
        if lamp_power is not None : path_save += 'lamp_power_{}'.format(lamp_power)
