...
def train_and_log_model(project_name, config):
    with wandb.init(project = project_name, job_type = "train", config = config) as run:
        ...
        if "VAE" in str(type(model)): # Chekc if it is a VAE/AE
            train_anomaly_model(model, optimizer, loader_list, config, lr_scheduler)
        elif "Sequence_embedding_clf" in str(type(model)): # Chekc if it is the sequence embedder
            train_sequence_embeddeding_clf_model(model, optimizer, loader_list, config, lr_scheduler)
        else:
            raise ValueError("Problem with the type of model you want to train")
